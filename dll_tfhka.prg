...
//oTFHKA:IFREPORTEZ()
   ...
   oTable:=OpenTable(" SELECT  MOV_CODIGO,INV_DESCRI,MOV_TOTAL,DOC_OTROS,DOC_DCTO,DOC_DESCCO,MOV_PRECIO,MOV_DESCUE,MOV_CANTID,MOV_IVA,CCG_NOMBRE,CCG_RIF,CCG_DIR1,CCG_TEL1"+;
...
FUNCTION IFABREFISCAL()
 ...
 IF oTable:DOC_TIPDOC = "TIK" .OR. oTable:DOC_TIPDOC="FAV"
    ...
    IF !Empty(oTable:DOC_DESCCO) .OR. !Empty(oTable:DOC_DESC)

         IF !Empty(oTable:DOC_DESCCO)
          lCadena:= CHR(105)+CHR(48)+CHR(48) + "%Dcto: " + oTable:DOC_DESCCO 
       ELSE
          lCadena:= CHR(105)+CHR(48)+CHR(48) + "%Dcto: " + LSTR(oTable:DOC_DESC)
       ENDIF
